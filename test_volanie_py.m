...
% Force MATLAB to reload the Python module 
% clear classes;
...
% py.radar_kod_pokus.collect_radar_data(cycles,range1,range2);
% fprintf('Capture finished. Loading data from CSV...\n');

% 3. Load the data from the CSV file generated by Python
M = readmatrix('radar_capture.csv');

% Verify the data is loaded
[rows, cols] = size(M);
disp(['Data loaded: ', num2str(rows), ' sweeps and ', num2str(cols), ' distance bins.']);

% --- NOISE ELIMINATION ---

% 1. Static Clutter Removal (Background Subtraction)
% Subtract the mean of each column from each row
mean_sweep = mean(M, 1);
M_no_clutter = M - mean_sweep;

% 2. Smoothing (2D Gaussian Filter)
% Adjust 1.0 to a higher number if it's still too noisy
M_denoised = imgaussfilt(M_no_clutter, 0.1);

% 3. Visualization
figure(1);
subplot(2,1,1);
imagesc(M); title('Raw Radar Data'); colorbar;
ylabel('Sweep #'); xlabel('Distance Bin');

subplot(2,1,2);
imagesc(M_denoised); title('Denoised Data (Clutter Removed + Gaussian)'); colorbar;
ylabel('Sweep #'); xlabel('Distance Bin');
